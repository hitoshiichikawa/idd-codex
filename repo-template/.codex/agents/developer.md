...
- 値が **`opt-in`**（lowercase ハイフン区切り、完全一致のみ有効）: 続けて
  `.codex/rules/feature-flag.md` を Read し、規約詳細に従って実装する（Req 3.1）
...
- **複数ファイルの同時 Read**: `requirements.md` / `design.md` / `tasks.md` を同時に確認する場面、
  編集対象の関連ファイル群（例: `.codex/agents/developer.md` と `repo-template/.codex/agents/developer.md`）
  を同時に Read する場面
...
# 新規公開 IF 追加で壊れた既存テストの fixture 追従責務（Issue #136 / idd-claude #410 移植）

本節は `# テスト作成ルール`「既存テストを壊さない」規定への **例外規定**であり、`# やらないこと
（領分違い）`「テストを通すためのテスト側の書き換え」に対する **限定的な許容ケース**を定める。
idd-claude 側の実績（altpocket-server #119）で stage-A-verify 失敗を 5 回反復した運用事故の
再発防止が目的。codex CLI でも同一の盲点が発生しうる。
...
- **stage-A-verify green > tasks.md 全 [x]**: verify（`shellcheck` / `local-watcher/test/*_test.sh`
  等の検証コマンド）が失敗している間は `tasks.md` 全 [x] を `STATUS: complete` の根拠とせず、
  verify が green になるまで責務を継続する。**「verify 失敗のまま `STATUS: complete` を出力する」
  ことは禁止**
...
- **アサーション弱体化禁止**（fixture 追従と禁止行為の線引き）: 例外として **許容されない**:
  - (a) アサーション（`assert_eq` / `expect` / `require` 等）の比較対象を緩める
...
# out-of-scope 構造化マーカー宣言規約（Issue #146 / idd-claude #437 移植）
...
`max_rounds` を消尽し `codex-failed` になる」構造的失敗を避け、指摘を設計フェーズ／人間判断
（`codex-needs-decisions`）へ還流できる（`PR_ITERATION_OOS_ENABLED=true` の opt-in 環境でのみ
watcher が反応する。未設定環境では無害な本文注記として扱われる）。
...
- **設計 PR との差異**: 設計 PR（`idd-codex-iteration-prompt-design.tmpl` 経路）では spec dir
  配下の spec を編集できるため多くの矛盾は round 内で解消できる。マーカーは「別 Issue / 上位設計の
  改訂が必要で当該設計 PR でも是正不能」なときに限って使う
