#!/usr/bin/env bash
# slack-notify.sh — 介入要求イベントの Slack 外部通知モジュール (#105 / D-18)
#
# 用途:
...
#   opt-in**、かつ `SLACK_WEBHOOK_URL` が設定されている場合のみ通知する。いずれか欠ける場合
#   （既定）は外部副作用ゼロの no-op（run-summary + ログのみ＝導入前と等価）。
#
...
#   入力: $1=event $2=kind $3=number $4=title $5=url
...
# ─────────────────────────────────────────────────────────────────────────────
sn_build_payload() {
  local event="$1" kind="$2" number="$3" title="$4" url="$5"
  jq -nc \
    --arg e "$event" --arg k "$kind" --arg n "$number" \
    --arg t "$title" --arg u "$url" --arg r "${REPO:-unknown/unknown}" \
    '{text: (":rotating_light: *idd-codex 介入要求* `" + $e + "` — <" + $u + "|" + $k + " #" + $n + ">: " + $t + " (" + $r + ")")}' \
    2>/dev/null
}
