#!/usr/bin/env bash
# auto-merge-disarm.sh — arm 済み auto-merge を terminal ラベル遷移時に取り消す processor（#145 / idd-claude #434 移植）
#
# 用途:
#   `auto-merge`（#99） / `auto-merge-design`（#100） processor が `gh pr merge --auto` で
#   arm した PR（`autoMergeRequest != null`）が、その後 `codex-failed` / `codex-needs-decisions`
#   といった terminal ラベルへ遷移しても disarm されず、必須 status checks が全 green に到達した
#   瞬間に「失敗確定済み PR」が誤って merge されてしまう不具合（Defect A / #145）を解消する。
...
#
# 配置先:
#   $HOME/bin/idd-codex-modules/auto-merge-disarm.sh（install.sh が
#   local-watcher/bin/idd-codex-modules/ から配置する）
#
# 依存:
...
#   - `idd_secure_mktemp` は core_utils.sh が定義（stderr 捕捉用の一時ファイル）。
#   - 外部 CLI: gh / jq。
#
# 後方互換性 (#145 / 新機能既定 OFF 規約):
...
#     （idd-claude #434 と同一の gate 設計。arm 源はいずれも既定 false のため
#      「新機能は既定 OFF」規約も同時に満たす）。
#
# セットアップ参照先:
#   README.md（「Auto-Merge Disarm Processor」節） / install.sh（配置ロジック）

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Auto-Merge Disarm Processor (#145)
#   FULL_AUTO_ENABLED AND (AUTO_MERGE_ENABLED OR AUTO_MERGE_DESIGN_ENABLED) の opt-in 下で、
#   arm 済み（autoMergeRequest != null）かつ terminal ラベル（codex-failed /
#   codex-needs-decisions）付きの open PR の native auto-merge を
#   `gh pr merge --disable-auto` で取り消す。
#   gate OFF / 非対象 PR では完全 no-op で本不具合修正導入前と等価。
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

# ロガー: 既存 am_log / amd_log と同形式の `[YYYY-MM-DD HH:MM:SS] [$REPO] auto-merge-disarm:` 形式。
amx_log() {
...
#
#   設計判断（gate の arm 源相乗り / idd-claude #434 と同一）:
...
#     arm 自体が起きないため、disarm も完全 no-op で後方互換。
#     さらに #97 kill switch（FULL_AUTO_ENABLED）との AND を取り、kill switch OFF では一切
#     発火しない（arm 側と同じ AND 二重 opt-in セマンティクス）。
#     専用の `AUTO_MERGE_DISARM_ENABLED` を新設しない理由: disarm は arm の安全弁であり、
#     「arm は ON だが disarm は OFF」という設定は失敗 PR が merge され得る危険状態を
#     再導入する。arm 源に相乗りさせることで設定ミスの余地を残さない。
#
#   値正規化: `AUTO_MERGE_ENABLED` / `AUTO_MERGE_DESIGN_ENABLED` は `=true` 厳密一致のみ ON。
#     未設定 / 空 / `false` / `0` / `True` / `TRUE` / `1` / `on` / `yes` / typo 等はすべて
#     OFF として扱う（安全側。本体 Config ブロックで正規化済みだが unset 経路でも安全に倒す）。
#     FULL_AUTO_ENABLED は full_auto_enabled に委譲。
#
#   戻り値: 0 = gate ON / 1 = OFF
#   副作用: なし（純粋関数）
# ─────────────────────────────────────────────────────────────────────────────
...
amx_should_disarm_for_pr() {
  ...
  # 未 arm（autoMergeRequest == null）は対象外（冪等 no-op の前提条件）
  ...
  # 未信頼値（ラベル名）は jq --arg で渡す（フィルタ文字列へ inline 展開しない）
...
#     万一 disable-auto を二重に打っても GitHub 側で no-op 相当（副作用最小）。
# ─────────────────────────────────────────────────────────────────────────────
amx_disarm_pr() {
  local pr_number="$1"
  local head_ref="${2:-}"
  ...
  # PR 番号は数値のみ（URL / gh 引数として使う直前に検証）
  ...
  # `--` でオプション解釈打ち切り（PR 番号は数値検証済みだが安全側で統一 / am_ と同型）。
  ...
  # disarm 失敗は WARN 1 行を残して fail-continue（パイプライン継続）。silent fail させない。
  local stderr_tail=""
  if [ -n "$stderr_file" ]; then
    stderr_tail=$(tail -c 512 "$stderr_file" 2>/dev/null | tr '\n' ' ')
    rm -f "$stderr_file"
  fi
...
#      （gh API ゼロ呼び出し = 導入前と等価）
#   2. open PR を gh pr list で取得（GitHub 直接クエリ。state dir に依存しない）。
#      head pattern は impl / design 双方の OR でクライアント側 jq フィルタ
#      （人間が手書きした PR を除外）+ fork 除外
#   3. 各 PR について amx_should_disarm_for_pr → amx_disarm_pr。
#      1 件失敗で全体を止めない（fail-continue）
#   4. サマリ 1 行を出力。対象 0 件は過剰ログを出さない
#
...
process_auto_merge_disarm() {
  ...
  # GitHub を直接クエリして open PR を取得（pending state dir には依存しない）。
  #   - state:open のみ取得。terminal ラベル / arm 有無は client 側 jq で判定する
  #     （autoMergeRequest は server-side filter 不可のため client 側で扱う）。
  ...
  # 未信頼値（pattern / owner）は jq --arg で渡す（フィルタ文字列へ inline 展開しない）。
  prs_json=$(printf '%s' "$prs_json" | jq -c \
    ...
    '[.[]
      | select((.headRefName // "") | (test($impl_pattern) or test($design_pattern)))
  ...
  if [ -n "$target_iter" ]; then
    ...
    while IFS= read -r pr_json; do
      ...
      # disarm 対象でなければ skip（gh 呼び出しなし / 冪等 no-op）
  ...
  # 対象 0 件のときは過剰ログを出さず、サマリ 1 行のみに留める。
  amx_log "auto-merge-disarm summary: disarmed=${disarmed_count} failed=${failed_count} open-candidates=${total}"
  return 0
}
