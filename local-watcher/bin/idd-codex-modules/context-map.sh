...
ci_collect_indexer_metadata_blocks() {
  local path="$1"
  local task_id="${2:-}"
  local stage="${3:-}"
  local range_key="${4:-}"
...
ci_record_indexer_marker() {
  ...
  {
    printf '\n'
    ci_print_indexer_status "<!-- context-indexer: task=${task_id} stage=${stage} range=${range_key} result=${result} reason=${reason_token} -->" "skip:already-run"
  } >> "$context_path"
}
...
cm_write_context_map() {
  ...
  doc_paths="$(printf '%s\n' "$all_paths" | cm_filter_context_paths doc)"
  range_key="$(ci_range_key "$range_start" "$range_end")"
  existing_indexer_markers="$(ci_collect_indexer_markers "$out_path" | awk -v task="$task_id" -v stage="$stage" -v range="$range_key" '
    index($0, " task=" task " ") && index($0, " stage=" stage " ") && index($0, " range=" range " ") { print }
  ')"
  existing_indexer_metadata="$(ci_collect_indexer_metadata_blocks "$out_path" "$task_id" "$stage" "$range_key")"
    ...
    case "$indexer_decision" in
      needed:*)
        if [ -n "$existing_indexer_markers" ]; then
          printf '\n'
          ci_print_indexer_status "$existing_indexer_markers" "$indexer_decision"
        fi
        ;;
      *)
        printf '\n'
...
cm_build_prompt_block() {
  ...
  stage="$(cm_extract_context_map_stage "$context_path" 2>/dev/null || true)"
  case "$stage" in
    reviewer)
