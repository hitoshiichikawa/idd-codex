...
# `codex-needs-decisions` へ還流させ、Developer 構造化マーカー（`OUT-OF-SCOPE: design` /
# `OUT-OF-SCOPE: spec-stale`）検出と内容ベース fingerprint の早期打ち切りで max_rounds 到達前に
# 停止する機能。「設計レベル指摘の堂々巡りで no-progress-streak がリセットされ続け、収束しない
# まま max_rounds を消尽して codex-failed に倒れる」事象を解消する。
#
# **完全な opt-in（既定 OFF）**: `PR_ITERATION_OOS_ENABLED=true` 厳密一致以外（未設定 / 空 /
# `True` / `TRUE` / `1` / `0` / typo 等）はすべて安全側＝無効として `false` に正規化し、本機能
# 導入前と完全に等価な挙動を保つ。gate OFF 時、adjudicator は `out-of-scope` を一切出さず
# 受け取らず（既存 2 値 schema と `legitimate + excessive == total` 不変条件を完全保持）、
# pr-iteration の oos フィルタ / Developer marker 検出 / 内容ベース早期打ち切りは発火せず
# （既存 SHA ベース streak のみ）、prompt template の out-of-scope 指示は注入されない。
# 既存 `PR_REVIEWER_ADJUDICATOR_ENABLED`（#138 で既定 ON）/ `PR_ITERATION_ENABLED`（#112 で
# 既定 ON）には相乗りせず**新規 env を新設**して no-op 既定を保証する（相乗りは ON 既定のため
# no-op を保証できない）。本 gate は既存 gate を override しない（後方互換優先）。
#
# 後方互換性:
#   - 既存 env var 名 / ラベル名（`codex-needs-decisions` 再利用 = 新ラベル新設なし）/
#     exit code / cron 文字列 / ログ prefix / 既存 marker フォーマットの round / last-run /
#     no-progress-streak キーは不変。本機能は新 env / 新 marker キー
#     （`oos-no-progress-streak` / `oos-fingerprint`）の追加のみ。
#
# 関数本体は idd-codex-modules/adjudicator.sh（adj_ prefix）/ idd-codex-modules/pr-iteration.sh
# （pi_ prefix）。
PR_ITERATION_OOS_ENABLED="${PR_ITERATION_OOS_ENABLED:-false}"
# 値正規化: `true` 厳密一致のみ通し、それ以外はすべて `false` に固定する（安全側）。
# 既定 OFF の opt-in 制のため、「デフォルト有効化フラグの値正規化」ループには加えない。
case "$PR_ITERATION_OOS_ENABLED" in
  true) : ;;
  *)    PR_ITERATION_OOS_ENABLED="false" ;;
esac
# out-of-scope 還流ルート。許容値は `needs-decisions`（推奨既定）/ `design-reflow` /
# `spawn-issue` の 3 値。本実装では `design-reflow` / `spawn-issue` も実処理は
# `needs-decisions`（= `codex-needs-decisions` ラベル）に丸める（env 値だけ将来予約 /
# 外部への新規 Issue 作成は opt-in gate 体系上の Non-Goal）。未知値 / 空 / typo は安全側と
# して `needs-decisions` に正規化する。
...
# 内容ベース早期打ち切りの上限 N。out-of-scope は確定的に収束不能なため、既存 SHA ベース
# no-progress 上限（既定 3）とは別軸で既定 2 と短く設定する。非数値 / 0 以下は安全側として
# 既定 2 に正規化する。gate OFF 時は本値を参照しない（早期打ち切り自体が no-op）。
