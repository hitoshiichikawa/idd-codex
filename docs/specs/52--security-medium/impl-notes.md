...
| 4.4 | `pr_run_review_for_pr` exec-failed generic detail | Public PR error comment body | `security_medium_pr_reviewer_test.sh`: public comment includes PR, sha, tool, exit code, correlation token | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass | correlation token は artifact basename。local path は公開しない |
...
| NFR 1.1 | `pr-reviewer.sh` existing env var reads unchanged | PR Reviewer configured by existing env vars | diff review; `security_medium_pr_reviewer_test.sh` uses existing `PR_REVIEWER_*` env names | security_medium full verify pass | no env var rename |
| NFR 1.2 | `pr-reviewer.sh` existing label constants and transitions unchanged | PR Reviewer quota / iteration / approval flows | `pr_reviewer_quota_marker_test.sh`; `pr_reviewer_approval_signal_test.sh` | both tests pass | label names and meanings preserved |
