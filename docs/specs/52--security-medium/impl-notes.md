...
- 採用方針: PR Reviewer の `{BASE}` / `{HEAD}` / `{PR}` placeholder 値を field 別 allowlist で検証し、non-quota exec failure の公開コメントから raw stdout / stderr を除去した。
- 重要な判断:
  - `PR_REVIEWER_*_CMD` は既存どおり trusted operator configuration とし、PR-derived 値だけを `pr_validate_placeholder_value` で data として検証する境界にした。
  - rejected placeholder は public comment を作らず、operator-visible warning も raw value ではなく PR number / field / reason category に限定した。
  - exec-failed の raw stdout / stderr は secure diagnostic artifact に保持し、public comment には PR number、sha、tool、exit code、correlation token だけを載せる。
  - 既存 `pr_reviewer_quota_marker_test.sh` / `pr_reviewer_approval_signal_test.sh` は task 5 以降の `idd_secure_mktemp` と task 6 の validation helper 依存を読み込むように更新した。
- 残存課題: なし（README / QUICK-HOWTO への operator-visible behavior 同期は task 7 scope）。

#### Finding Closure Matrix

| Target requirement | Category | Required Action | Fix commit | Test/assertion | Verification result | Notes / no-change reason |
|--------------------|----------|-----------------|------------|----------------|---------------------|--------------------------|
| N/A | N/A | Reviewer reject / Debugger 指摘なし | `7742910 fix(watcher): harden pr reviewer placeholders` | N/A | N/A | `review-notes.md` は task 5 approve のみで、task 6 の reject Finding は存在しない。 |

### Task 7

- 採用方針: 実装済み hardening の operator-visible behavior を README / QUICK-HOWTO に同期し、既存 bootstrap docs regression に key phrase assertions を追加した。
- 重要な判断:
  - README には local runtime `.bak` recovery、dry-run action、Guard profile exact path handling、secure tempfile fail-closed、PR Reviewer generic exec-failed public comment、既存 env / label / watcher path 不変の migration note を集約した。
  - QUICK-HOWTO は最短導入手順の文脈を崩さない範囲で、mutable branch override / checksum verification / local runtime recovery の要点だけを補足した。
  - 新規 shell 実装は追加せず、docs 同期 regression は `security_medium_bootstrap_docs_test.sh` に閉じた。
- 残存課題: なし（task 8 の full static verify / smoke は別 task scope）。
...
| 1.4 | `README.md`, `QUICK-HOWTO.md` override note | Operator が mutable branch override を選ぶ docs flow | `security_medium_bootstrap_docs_test.sh`: README / QUICK-HOWTO document `IDD_CODEX_BRANCH=main` override | `bash local-watcher/test/security_medium_bootstrap_docs_test.sh` pass | mutable branch は明示 override として許容 |
| 1.5 | `README.md`, `QUICK-HOWTO.md` checksum verification note | Operator が release artifact を手動検証する docs flow | `security_medium_bootstrap_docs_test.sh`: checksum verification path in README / QUICK-HOWTO | `bash local-watcher/test/security_medium_bootstrap_docs_test.sh` pass | checksum artifact 生成は本 task scope 外。QUICK-HOWTO にも保存後検証 flow を明記 |
...
| 2.3 | `install.sh`: `log_action BACKUP`, `.bak` recovery path; `README.md`, `QUICK-HOWTO.md` recovery docs | Operator が install log / docs を確認して previous contents を復元する flow | `security_medium_install_test.sh`: watcher / plist backup path is operator-visible; `security_medium_bootstrap_docs_test.sh`: README / QUICK-HOWTO document local runtime recovery | `bash local-watcher/test/security_medium_install_test.sh` pass; `bash local-watcher/test/security_medium_bootstrap_docs_test.sh` pass | recovery は `<target>.bak` |
...
| 5.3 | `core_utils.sh`: fail-closed branches in `idd_secure_mktemp`; watcher call-sites using `|| return` or explicit mark-failed path; `README.md` secure tempfile policy | Secure tempfile creation failure in current watcher operation | `security_medium_tempfiles_test.sh`: uncreatable tmp root returns non-zero and emits `secure-tempfile: ERROR:`; no predictable fallback path is created; `security_medium_bootstrap_docs_test.sh`: README documents secure tempfile fail-closed policy | `bash local-watcher/test/security_medium_tempfiles_test.sh` pass; `bash local-watcher/test/security_medium_bootstrap_docs_test.sh` pass | push verify / resume push call-sites additionally route through existing failure paths |
...
| 4.1 | `pr-reviewer.sh`: non-quota exec failure branch in `pr_run_review_for_pr` | `PR_REVIEWER_ENABLED=true` で候補 PR を review command に渡し、command が non-zero exit した flow | `security_medium_pr_reviewer_test.sh`: public comment excludes stderr local path / token-like stderr / stdout excerpt | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass; security_medium full verify pass | quota reset path は `pr_detect_usage_limit_reset_epoch` で既存どおり分岐し、non-quota のみ generic comment |
| 4.2 | `pr_write_exec_failure_diagnostic`; `README.md` PR Reviewer docs | PR Reviewer review command non-zero exit の local diagnostic retention flow | `security_medium_pr_reviewer_test.sh`: local log records retained diagnostic path / diagnostic retains stderr and stdout detail; `security_medium_bootstrap_docs_test.sh`: README documents generic public comment and local diagnostics | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass; `bash local-watcher/test/security_medium_bootstrap_docs_test.sh` pass | diagnostic artifact は `idd_secure_mktemp` 配下に意図的保持し、public body には path ではなく basename correlation token のみ載せる |
| 4.3 | `pr_run_review_for_pr` exec-failed detail construction | Public PR error comment posting flow via `pr_post_error_comment` | `security_medium_pr_reviewer_test.sh`: excludes `/home/operator`, `ghp_secret_should_stay_local`, and stdout sentinel from public comment | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass | raw stdout / stderr は local artifact 限定 |
| 4.4 | `pr_run_review_for_pr` exec-failed generic detail; `README.md` PR Reviewer docs | Public PR error comment body | `security_medium_pr_reviewer_test.sh`: public comment includes PR, sha, tool, exit code, correlation token; `security_medium_bootstrap_docs_test.sh`: README documents generic exec-failed public comment | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass; `bash local-watcher/test/security_medium_bootstrap_docs_test.sh` pass | correlation token は artifact basename。local path は公開しない |
| 4.5 | `process_pr_reviewer` opt-in gate | `PR_REVIEWER_ENABLED!=true` processor entrypoint | `security_medium_pr_reviewer_test.sh`: disabled processor does not fetch PRs / does not post comments | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass | no-op behavior preserved |
| 6.1 | `pr_validate_placeholder_value`, `pr_substitute_placeholders` | Review command placeholder substitution before `bash -c "$resolved_cmd"` | `security_medium_pr_reviewer_test.sh`: normal placeholders substituted; unsafe placeholder does not execute review command | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass | command template itself remains trusted operator config by design |
| 6.2 | `pr_placeholder_reject_reason`, `pr_validate_placeholder_value` | Candidate PR processing in `pr_run_review_for_pr` before command execution | `security_medium_pr_reviewer_test.sh`: newline head rejected; glob / leading dash / non-numeric PR reason assertions; unsafe value returns skip/failure | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass | warning uses PR number / field / reason category without raw value |
| 6.3 | `pr_fetch_candidate_prs` unchanged head pattern filter | PR Reviewer candidate fetch flow | `security_medium_pr_reviewer_test.sh`: head pattern and fork exclusion leave only same-owner `codex/` PR | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass | existing `PR_REVIEWER_HEAD_PATTERN` semantics preserved |
| 6.4 | `pr_fetch_candidate_prs` unchanged same-owner filter | PR Reviewer candidate fetch flow | `security_medium_pr_reviewer_test.sh`: fork owner excluded from candidate list | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass | fork PR exclusion preserved |
| 6.5 | `pr_validate_placeholder_value`; exec-failed public comment detail | Unsafe placeholder skip and public error comment flow | `security_medium_pr_reviewer_test.sh`: unsafe placeholder posts no public comment and warning omits raw value; exec-failed public comment omits raw diagnostics | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass | rejected placeholder values are local warning only |
| NFR 1.1 | `pr-reviewer.sh` existing env var reads unchanged; `README.md` migration note | PR Reviewer configured by existing env vars | diff review; `security_medium_pr_reviewer_test.sh` uses existing `PR_REVIEWER_*` env names; `security_medium_bootstrap_docs_test.sh`: README documents unchanged env names | security_medium full verify pass; `bash local-watcher/test/security_medium_bootstrap_docs_test.sh` pass | no env var rename |
| NFR 1.2 | `pr-reviewer.sh` existing label constants and transitions unchanged; `README.md` migration note | PR Reviewer quota / iteration / approval flows | `pr_reviewer_quota_marker_test.sh`; `pr_reviewer_approval_signal_test.sh`; `security_medium_bootstrap_docs_test.sh`: README documents unchanged labels | both tests pass; `bash local-watcher/test/security_medium_bootstrap_docs_test.sh` pass | label names and meanings preserved |
| NFR 2.2 | `pr_validate_placeholder_value`, `pr_write_exec_failure_diagnostic` | Placeholder rejection and exec failure diagnostic flows | `security_medium_pr_reviewer_test.sh`: warning includes reason category; local log records diagnostic path | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass | fail-closed / reject cases are operator-visible |
| NFR 2.3 | `security_medium_pr_reviewer_test.sh` | PR Reviewer placeholder / redaction regression suite | normal substitution, unwanted newline/glob/non-numeric/leading dash, redaction, disabled no-op, head pattern / fork exclusion | `bash local-watcher/test/security_medium_pr_reviewer_test.sh` pass; `shellcheck ... security_medium_pr_reviewer_test.sh` pass | task 6 の正常系・異常系・境界値を含む |
...
| NFR 2.1 | `README.md`, `QUICK-HOWTO.md` task 7 docs sync | Operator が changed hardening behavior を読む docs flow | `security_medium_bootstrap_docs_test.sh`: local runtime recovery, dry-run actions, Guard profile exact path handling, secure tempfile policy, PR Reviewer generic exec-failed comment, migration note | `bash local-watcher/test/security_medium_bootstrap_docs_test.sh` pass; `shellcheck setup.sh local-watcher/test/security_medium_bootstrap_docs_test.sh` pass | task 7 で実装済み挙動を docs に同期 |
...
- `bash local-watcher/test/security_medium_pr_reviewer_test.sh` — pass (task 6)
- `bash local-watcher/test/pr_reviewer_quota_marker_test.sh` — pass (task 6 non-regression)
- `bash local-watcher/test/pr_reviewer_approval_signal_test.sh` — pass (task 6 non-regression)
- `shellcheck local-watcher/bin/idd-codex-modules/pr-reviewer.sh local-watcher/test/security_medium_pr_reviewer_test.sh local-watcher/test/pr_reviewer_quota_marker_test.sh local-watcher/test/pr_reviewer_approval_signal_test.sh` — pass (task 6)
- `shellcheck setup.sh install.sh local-watcher/bin/idd-codex-issue-watcher.sh local-watcher/bin/idd-codex-modules/core_utils.sh local-watcher/bin/idd-codex-modules/auto-rebase.sh local-watcher/bin/idd-codex-modules/pr-reviewer.sh local-watcher/bin/idd-codex-modules/pr-iteration.sh local-watcher/bin/idd-codex-modules/quota-aware.sh local-watcher/test/security_medium_bootstrap_docs_test.sh local-watcher/test/security_medium_install_test.sh local-watcher/test/security_medium_tempfiles_test.sh local-watcher/test/security_medium_pr_reviewer_test.sh && bash local-watcher/test/security_medium_bootstrap_docs_test.sh && bash local-watcher/test/security_medium_install_test.sh && bash local-watcher/test/security_medium_tempfiles_test.sh && bash local-watcher/test/security_medium_pr_reviewer_test.sh` — pass (task 6 security_medium verify)
